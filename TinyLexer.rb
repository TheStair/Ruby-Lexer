...
def nextToken()
	if @c == "eof"
		...
		if str == "print"
			...
			tok = Token.new(Token::WLOOP, str)
			return tok
		elsif str == "for"
			tok = Token.new(Token::FLOOP, str)
			return tok
		else
		...
		str = ">"
		tok = Token.new(Token::GRTR,str)
		nextCh
		return tok

	elsif	@c == "<"
		str = "<"
		tok = Token.new(Token::LESS,str)
		nextCh
		return tok
		...
		nextCh
		return "Found Unknown Lexeme: " + tok
	end
